$(document).ready(function () {
    if(!$("#id_funcionario_coleta").is(":checked") == true){
        $(".funcionario_coleta_hidden").css("display", "none");
    }
    ...
    $('#div_procedimento input.search').keyup(function () {
        if (($("div[id='div_procedimento'] input[class='search']").val()).length >= 3) {
            $.ajax({
                type: 'POST',
                url: '/coleta/busca_procedimentos_medicos/',
                data: {
...
//     $.ajax({
//         type: 'POST',
//         url: '/coleta/escolha_departamento/',
//         data: {
...
function get_cidade_natural() {
    $.ajax({
        type: 'POST',
        url: '/coleta/escolha_cidade_natural/',
        data: {
...
function get_cidade_atual() {
    $.ajax({
        type: 'POST',
        url: '/coleta/escolha_cidade_atual/',
        data: {
...
function get_cidade_local_trabalho() {
    $.ajax({
        type: 'POST',
        url: '/coleta/escolha_cidade_local_trabalho/',
        data: {
...
$('#id_funcionario_coleta').click(function () {
    if($(".funcionario_coleta_hidden").is(":hidden")){
        $(".funcionario_coleta_hidden").css("display", "inline");
    }else{
        $(".funcionario_coleta_hidden").css("display", "none");
    }
});
...
function modalRequerimento(id) {
    html = "<div class='ui green ok inverted button'>Não</div>" +
        "<a class='ui red ok inverted button IDAgendamento' href='/coleta/deleta/requerimento_sem_agendamento/"
...
function atualiza_select_secretaria(id_secretaria) {
    $.ajax({
        type: 'POST',
        url: '/coleta/atualiza/secretaria/',
        data: {
...
function atualiza_select_local_trabalho(id_local_trabalho) {
    $.ajax({
        type: 'POST',
        url: '/coleta/atualiza/local_trabalho/',
        data: {
